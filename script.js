...
/* MI_DIRECT_CATEGORY_OPEN_START */

(function () {
    "use strict";

    if (window.__miDirectCategoryOpenInstalled) {
        return;
    }

    window.__miDirectCategoryOpenInstalled = true;
    ...
    function getRequestedPage(control) {
        if (!control) {
            return "";
        }
        ...
        if (validPages.has(directPage.toLowerCase())) {
            return directPage.toLowerCase();
        }

        const href = control.getAttribute("href") || "";

        const hashValue = href
            .replace(/^.*#/, "")
            .split("/")[0]
            .trim()
            .toLowerCase();

        if (validPages.has(hashValue)) {
            return hashValue;
        }

        const text = (
            control.querySelector(
                ".mcx-main-directory-title, h2, h3, strong"
            )?.textContent ||
            control.textContent ||
            ""
        )
    ...
    function getPageSection(pageName) {
        return (
    ...
    function hideFrontPage() {
        const frontPage =
            ...
            document.querySelector(".mi-front-page") ||
            document.querySelector("[data-front-page]");
    ...
    function showCategoryDirectly(pageName) {
        ...
        allPages.forEach(function (page) {
            ...
            const isSelected = pageId === pageName;

            page.classList.toggle("active", isSelected);
        ...
        const requestedPage = getPageSection(pageName);
        ...
        requestedPage.setAttribute("aria-hidden", "false");

        requestedPage.style.removeProperty("display");
        requestedPage.style.removeProperty("visibility");
        requestedPage.style.removeProperty("opacity");
        requestedPage.style.removeProperty("transform");
        ...
        const heading = requestedPage.querySelector("h1, h2");
    ...
    function isMainDirectoryControl(element) {
        return Boolean(
            element.closest(
                ".mcx-main-directory, " +
                ".mcx-main-portal, " +
    ...
    document.addEventListener(
        ...
        function (event) {
            ...
            if (!control || !isMainDirectoryControl(control)) {
                return;
            }

            const requestedPage = getRequestedPage(control);
            ...
            showCategoryDirectly(requestedPage);
        },
        true
    );
    ...
    window.addEventListener("popstate", function () {
        ...
        if (validPages.has(pageName)) {
            showCategoryDirectly(pageName);
        }
    });
})();

/* MI_DIRECT_CATEGORY_OPEN_END */

...
/* MI_CATEGORY_SCROLL_TARGET_START */

(function () {
    "use strict";

    if (window.__miCategoryScrollTargetInstalled) {
        return;
    }

    window.__miCategoryScrollTargetInstalled = true;
    ...
    function detectPage(control) {
        ...
        const directValue = (
            ...
            control.dataset.mainCategory ||
            control.dataset.targetPage ||
            ""
        )
            .trim()
            .toLowerCase();

        if (validPages.has(directValue)) {
            return directValue;
        }
        ...
        if (validPages.has(hrefPage)) {
            return hrefPage;
        }

        const text = (
            control.querySelector(
                "h1, h2, h3, strong, .mcx-main-directory-title"
            )?.textContent ||
            control.textContent ||
            ""
        )
    ...
    function findCategorySection(pageName) {
        return (
            ...
            document.querySelector(
                `[data-mcx-dynamic-page="${pageName}"]`
            ) ||
            document.querySelector(
                `[data-main-category-page="${pageName}"]`
            )
        );
    }
    ...
    function getNavigationOffset() {
        ...
        return navigation.getBoundingClientRect().height + 22;
    }

    function scrollToSelectedCategory(pageName) {
        const categorySection = findCategorySection(pageName);
        ...
        categorySection.removeAttribute("aria-hidden");
        ...
        const heading = categorySection.querySelector("h1, h2");
    ...
    function scheduleCategoryScroll(pageName) {
        /*
        Existing website JavaScript first opens the category.
        This code only moves the viewport to that category afterwards.
        The front page remains visible above the category.
        */

        window.setTimeout(function () {
            scrollToSelectedCategory(pageName);
        }, 80);
    ...
    document.addEventListener(
        ...
        function (event) {
            const control = event.target.closest(
                ".mcx-main-directory-card, " +
                ".mcx-main-portal-card, " +
                ".main-category-card, " +
                "[data-main-category], " +
                "[data-category-card], " +
                "nav [data-page], " +
            ...
            const pageName = detectPage(control);
            ...
            /*
            Do not cancel the website's existing click handler.
            Let it open the correct category first.
            */

            scheduleCategoryScroll(pageName);
        },
        false
    );

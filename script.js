...
// Ensure a single navigation controller flag exists early.
// This signals other modules to short-circuit legacy routing handlers
// so the canonical `route()` in this file becomes the source of truth.
try {
	if (!window.__MCX_NAV_CONTROLLER_INITIALIZED__) {
		window.__MCX_NAVIGATION_CONTROLLER__ = true;
		window.__MCX_NAV_CONTROLLER_INITIALIZED__ = true;
	}
} catch (e) {}
...
(() => {
 ...
 function hashState() { const raw = location.hash.replace(/^#\/?/, "") || "overview"; const [pageRaw = "overview", category = "", subtopic = ""] = raw.split("/"); const canonicalPage = pageRaw === "overview"? "home": pageRaw; const page = validPages.includes(canonicalPage)? canonicalPage: "home"; return { page, category, subtopic }; }
 function setHash(page, category = "", subtopic = "") { const normalizedPage = page === "overview"? "home": page; const next = `#/${normalizedPage}${category? `/${category}`: ""}${subtopic? `/${subtopic}`: ""}`; if (location.hash === next) route(); else location.hash = next; }
 function hideAllCategoryPanels() {
 document.querySelectorAll("[data-mcx-category-index], [data-mcx-category-detail]").forEach((panel) => {
 panel.hidden = true;
 panel.innerHTML = "";
 });
 }
...
...premiumPage.querySelectorAll("*")
 ...
 function route(scroll = true) { const { page, category, subtopic } = hashState(); const resolvedPage = page === "overview"? "home": page; const activePage = document.querySelector(`[data-mcx-page="${resolvedPage}"]`); document.querySelectorAll("[data-mcx-page]").forEach((section) => { const active = section.dataset.mcxPage === resolvedPage; section.classList.toggle("active", active); section.hidden = !active; }); document.querySelectorAll("[data-mcx-page-link]").forEach((link) => { const linkPage = String(link.dataset.mcxPageLink || "").trim().toLowerCase(); const active = linkPage === resolvedPage; link.classList.toggle("active", active); if (active) { link.setAttribute("aria-current", "page"); } else { link.removeAttribute("aria-current"); } }); const index = document.querySelector(`[data-mcx-category-index="${resolvedPage}"]`); const detail = document.querySelector(`[data-mcx-category-detail="${resolvedPage}"]`); if (!category) { hideAllCategoryPanels(); } else if (["products", "services", "pricing", "premium"].includes(resolvedPage)) { if (category) { const item = getCatalogueItem(resolvedPage, category); if (item) { renderCatalogueDetail(resolvedPage, item); } else { if (index) index.hidden = false; if (detail) { detail.hidden = false; detail.innerHTML = `<div class="mcx-not-found"><h2>Not Found</h2><p>The requested catalogue item is not available yet. Please return to the main catalogue and choose another item.</p><button type="button" class="mcx-action" data-mcx-route="${resolvedPage}">BACK TO CATALOGUE</button></div>`; } } } } else if (category && renderCategory(resolvedPage, category, subtopic)) { } else { if (index) index.hidden = false; if (detail) { detail.hidden = true; detail.innerHTML = ""; } renderIndex(resolvedPage); } if (scroll && activePage) { const fixedNavigation = document.querySelector(".site-header") || document.querySelector("header") || document.querySelector("nav"); let navigationOffset = 22; if (fixedNavigation) { const navigationStyle = window.getComputedStyle(fixedNavigation); if (navigationStyle.position === "fixed" || navigationStyle.position === "sticky") navigationOffset = fixedNavigation.getBoundingClientRect().height + 22; } const targetTop = activePage.getBoundingClientRect().top + window.scrollY - navigationOffset; window.scrollTo({ top: Math.max(0, targetTop), left: 0, behavior: "smooth" }); }

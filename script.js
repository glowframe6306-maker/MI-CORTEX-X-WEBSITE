...
(() => {
  ...
  function formatCurrencyPrice(value) { return formatLkrPrice(value); }
  function getPriceLabel(item) { const priceText = formatLkrPrice(item.priceLkr); return `${item.prefix || "Starting from"} ${priceText}${item.billingPeriod ? ` / ${item.billingPeriod}` : ""}`; }
  function formatPriceForModal(item) { return formatLkrPrice(item.priceLkr); }

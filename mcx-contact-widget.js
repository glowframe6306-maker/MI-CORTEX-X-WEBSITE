...
(function () {
  ...
  function removeWidgetLabel() {
    ...
    elements.forEach((element) => {
      ...
      if (
        text === "connect with mi cortex x" &&
        element.children.length === 0
      ) {
        element.classList.add("mcx-hide-connect-label");
        element.setAttribute("aria-hidden", "true");
      }
    });
  }

  function applyIcon(launcher) {
    if (!launcher) {
      return;
    }
    ...
    launcher.classList.add(
      "mcx-custom-cycle-launcher",
      iconClasses[currentIcon]
    );
  ...
  function startIconCycle(launcher) {
    if (!launcher) {
      return;
    }
    ...
    cycleTimer = window.setInterval(() => {
      currentIcon =
        (currentIcon + 1) % iconClasses.length;
  ...
  function syncWidgetVisibility() {
    ...
    const root = findWidgetRoot(launcher);
    ...
    root.classList.toggle(
      "mcx-widget-hidden-on-front",
      isFrontPage()
    );
  }

  function initializeWidgetVisuals() {
    const launcher = findLauncher();

    if (!launcher) {
      window.setTimeout(
        initializeWidgetVisuals,
        250
      );
      return;
    }
    ...
    removeWidgetLabel();
    startIconCycle(launcher);
    syncWidgetVisibility();

    window.setTimeout(removeWidgetLabel, 500);
    window.setTimeout(removeWidgetLabel, 1500);
  }
